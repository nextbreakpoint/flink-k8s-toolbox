...
Build Flink jobs image:
    ...
    docker build -t example/flink-jobs:1 example/flink-jobs --build-arg repository=nextbreakpoint/flink-k8s-toolbox --build-arg version=1.3.4-beta
...
Optionally tag and push Docker image to your local Docker registry:
    ...
    docker tag flink-k8s-toolbox:1.3.4-beta registry:30000/flink-k8s-toolbox:1.3.4-beta
    docker login registry:30000
    docker push registry:30000/flink-k8s-toolbox:1.3.4-beta

Run Flink Operator using Docker image:

    kubectl run flink-operator --restart=Never -n flink --image=registry:30000/flink-k8s-toolbox:1.3.4-beta --overrides='{ "apiVersion": "v1", "metadata": { "labels": { "app": "flink-operator" } }, "spec": { "serviceAccountName": "flink-operator", "imagePullPolicy": "Always" } }' -- operator run --namespace=flink
