...
object ComputeMaximum {
  ...
  private def createExecutionEnvironment(developerMode: Boolean, restPort: Int, parallelism: Int) = {
    if (developerMode) {
      ...
      configuration.setString("state.checkpoints.dir", System.getProperty("state.checkpoints-path", "file:///tmp/checkpoints"))
      configuration.setString("state.savepoints.dir", System.getProperty("state.savepoints-path", "file:///tmp/savepoints"))
      configuration.setString("s3.access-key", System.getenv("S3_ACCESS_KEY"))
      configuration.setString("s3.secret-key", System.getenv("S3_SECRET_KEY"))
