...
class RequestSavepointForget(flinkOptions: FlinkOptions, flinkClient: FlinkClient, kubeClient: KubeClient, private val context: JobContext) : JobAction<Void?, Void?>(flinkOptions, flinkClient, kubeClient) {
    companion object {
    ...
    override fun execute(namespace: String, clusterName: String, jobName: String, params: Void?): Result<Void?> {
        return try {
            context.setJobManualAction(Action.FORGET_SAVEPOINT)
            ...
            logger.log(Level.SEVERE, "Can't forget savepoint", e)
