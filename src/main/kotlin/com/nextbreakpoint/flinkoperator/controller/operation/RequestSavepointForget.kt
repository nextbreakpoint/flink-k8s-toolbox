...
class RequestSavepointForget(flinkOptions: FlinkOptions, flinkClient: FlinkClient, kubeClient: KubeClient, private val bridge: CacheBridge) : Operation<Void?, Void?>(flinkOptions, flinkClient, kubeClient) {
    companion object {
    ...
    override fun execute(clusterId: ClusterId, params: Void?): OperationResult<Void?> {
        try {
            bridge.setManualAction(ManualAction.FORGET_SAVEPOINT)
