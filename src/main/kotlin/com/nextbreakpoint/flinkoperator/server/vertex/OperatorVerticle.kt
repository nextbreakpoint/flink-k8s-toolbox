package com.nextbreakpoint.flinkoperator.server.vertex
...
import com.nextbreakpoint.flinkoperator.server.operator.Operator
import com.nextbreakpoint.flinkoperator.server.operator.core.Cache
import com.nextbreakpoint.flinkoperator.server.operator.core.CacheAdapter
import com.nextbreakpoint.flinkoperator.server.common.Status
import com.nextbreakpoint.flinkoperator.server.controller.action.JobDetails
import com.nextbreakpoint.flinkoperator.server.controller.action.JobManagerMetrics
import com.nextbreakpoint.flinkoperator.server.controller.action.JobMetrics
import com.nextbreakpoint.flinkoperator.server.controller.action.TaskManagerDetails
import com.nextbreakpoint.flinkoperator.server.controller.action.TaskManagerMetrics
import com.nextbreakpoint.flinkoperator.server.controller.action.TaskManagersList
import com.nextbreakpoint.flinkoperator.server.vertex.core.Command
import com.nextbreakpoint.flinkoperator.server.vertex.core.Timeout
...
class OperatorVerticle : AbstractVerticle() {
    ...
    private fun createServer(config: JsonObject): Single<HttpServer> {
        ...
        val operatorCache = Cache()

        val cacheAdapter = CacheAdapter(kubeClient, operatorCache)

        val controller = Controller(flinkOptions, flinkClient, kubeClient)
                ...
                Function {
                    json.serialize(
                        controller.requestStartCluster(
                            it.clusterSelector, json.deserialize(it.json, StartOptions::class.java), ControllerContext(operatorCache.getFlinkCluster(it.clusterSelector))
                        )
                    )
                }
            )
                ...
                Function {
                    json.serialize(
                        controller.requestStopCluster(
                            it.clusterSelector, json.deserialize(it.json, StopOptions::class.java), ControllerContext(operatorCache.getFlinkCluster(it.clusterSelector))
                        )
                    )
                }
            )
                ...
                Function {
                    json.serialize(
                        controller.createSavepoint(
                            it.clusterSelector, ControllerContext(operatorCache.getFlinkCluster(it.clusterSelector))
                        )
                    )
                }
            )
    ...
    private fun makeV1FlinkCluster(context: RoutingContext, namespace: String): V1FlinkCluster {
        ...
        val flinkClusterSpec = Resource.parseV1FlinkClusterSpec(context.bodyAsString)
    ...
    private fun reconcileResources(controller: Controller, cache: Cache) {
        val resources = cache.getClusterSelectors().map {
