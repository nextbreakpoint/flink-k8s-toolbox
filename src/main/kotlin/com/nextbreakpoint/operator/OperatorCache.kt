...
class OperatorCache {
    ...
    fun onFlinkClusterChanged(resource: V1FlinkCluster) {
        flinkClusters.put(ClusterId(namespace = resource.metadata.namespace, name = resource.metadata.name, uuid = resource.metadata.uid), resource)
    }

    fun onFlinkClusterDeleted(resource: V1FlinkCluster) {
        flinkClusters.remove(ClusterId(namespace = resource.metadata.namespace, name = resource.metadata.name, uuid = resource.metadata.uid))
    }

    fun onServiceChanged(resource: V1Service) {
        jobmanagerServices.put(ClusterId(namespace = resource.metadata.namespace, name = extractClusterName(resource.metadata), uuid = extractClusterId(resource.metadata)), resource)
    }

    fun onServiceDeleted(resource: V1Service) {
        jobmanagerServices.remove(ClusterId(namespace = resource.metadata.namespace, name = extractClusterName(resource.metadata), uuid = extractClusterId(resource.metadata)))
    }

    fun onJobChanged(resource: V1Job) {
        jarUploadJobs.put(ClusterId(namespace = resource.metadata.namespace, name = extractClusterName(resource.metadata), uuid = extractClusterId(resource.metadata)), resource)
    }

    fun onJobDeleted(resource: V1Job) {
        jarUploadJobs.remove(ClusterId(namespace = resource.metadata.namespace, name = extractClusterName(resource.metadata), uuid = extractClusterId(resource.metadata)))
    }
