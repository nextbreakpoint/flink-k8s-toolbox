...
class ClusterResourcesStatusEvaluator {
    ...
    private fun evaluateUploadJobStatus(
        ...
        if (jarUploadJob.spec.template.spec.containers.size != 1) {
            ...
            if (container.image != flinkCluster.spec.flinkJobSpec.image) {
            ...
            if (container.args.size < 1 || container.args[0] != "upload") {
                ...
                if (container.args.size < 2 || container.args[1] != "jar") {
                    ...
                    if (jobJarPath == null || jobJarPath != flinkCluster.spec.flinkJobSpec.jarPath) {
    ...
    private fun evaluateJobManagerServiceStatus(
        ...
        if (jobmanagerService.spec.type != flinkCluster.spec.jobManagerSpec.serviceMode ?: "ClusterIP") {
    ...
    private fun evaluateJobManagerStatefulSetStatus(
        ...
        if (jobmanagerStatefulSet.spec.volumeClaimTemplates.size != 1) {
            ...
            if (jobmanagerStatefulSet.spec.volumeClaimTemplates[0].spec.storageClassName != flinkCluster.spec.jobManagerSpec.storageClass ?: "standard") {
                statusReport.add("volume claim storage class doesn't match")
            }

            if (jobmanagerStatefulSet.spec.volumeClaimTemplates[0].spec.resources.requests.get("storage")?.number?.toInt()?.equals(flinkCluster.spec.jobManagerSpec.requiredStorageSize ?: 1) != true) {
        ...
        if (jobmanagerStatefulSet.spec.template.spec.containers.size != 1) {
            ...
            if (container.resources.limits.get("cpu")?.number?.toFloat()?.equals(flinkCluster.spec.jobManagerSpec.requiredCPUs ?: 1.0f) != true) {
                statusReport.add("container cpu limit doesn't match")
            }

            if (container.resources.requests.get("memory")?.number?.toInt()?.equals((flinkCluster.spec.jobManagerSpec.requiredMemory ?: 256) * 1024 * 1024) != true) {
            ...
            if (jobmanagerMemoryEnvVar?.value == null || jobmanagerMemoryEnvVar.value.toInt() < flinkCluster.spec.jobManagerSpec.requiredMemory ?: 256) {
            ...
            if (jobmanagerEnvironmentVariables != flinkCluster.spec.jobManagerSpec.environment ?: listOf<V1EnvVar>()) {
    ...
    private fun evaluateTaskManagerStatefulSetStatus(
        ...
        if (taskmanagerStatefulSet.spec.volumeClaimTemplates.size != 1) {
            ...
            if (taskmanagerStatefulSet.spec.volumeClaimTemplates[0].spec.storageClassName != flinkCluster.spec.taskManagerSpec.storageClass ?: "standard") {
                statusReport.add("volume claim storage class doesn't match")
            }

            if (taskmanagerStatefulSet.spec.volumeClaimTemplates[0].spec.resources.requests.get("storage")?.number?.toInt()?.equals(flinkCluster.spec.taskManagerSpec.requiredStorageSize ?: 5) != true) {
                statusReport.add("volume claim size doesn't match")
            }
        }

        if (taskmanagerStatefulSet.spec.replicas != flinkCluster.spec.taskManagerSpec.replicas ?: 1) {
        ...
        if (taskmanagerStatefulSet.spec.template.spec.containers.size != 1) {
            ...
            if (container.resources.limits.get("cpu")?.number?.toFloat()?.equals(flinkCluster.spec.taskManagerSpec.requiredCPUs ?: 1.0f) != true) {
                statusReport.add("container cpu limit doesn't match")
            }

            if (container.resources.requests.get("memory")?.number?.toInt()?.equals((flinkCluster.spec.taskManagerSpec.requiredMemory ?: 1024) * 1024 * 1024) != true) {
                statusReport.add("container memory limit doesn't match")
            }
            ...
            if (taskmanagerMemoryEnvVar?.value == null || taskmanagerMemoryEnvVar.value.toInt() < flinkCluster.spec.taskManagerSpec.requiredMemory ?: 1024) {
            ...
            if (taskmanagerTaskSlotsEnvVar?.value == null || taskmanagerTaskSlotsEnvVar.value.toInt() != flinkCluster.spec.taskManagerSpec.taskSlots ?: 1) {
            ...
            if (!taskmanagerEnvironmentVariables.equals(flinkCluster.spec.taskManagerSpec.environment ?: listOf<V1EnvVar>())) {
                statusReport.add("container environment variables don't match")
            }
        }
    ...
    private fun evaluateJobManagerPersistentVolumeClaimStatus(
        ...
        if (jobmanagerPersistentVolumeClaim.spec.storageClassName != flinkCluster.spec.jobManagerSpec.storageClass ?: "standard") {
    ...
    private fun evaluateTaskManagerPersistentVolumeClaimStatus(
        ...
        if (taskmanagerPersistentVolumeClaim.spec.storageClassName != flinkCluster.spec.taskManagerSpec.storageClass ?: "standard") {
            statusReport.add("persistent volume storage class doesn't match")
        }
