...
@ExtendWith(EnsureBehaviourTest.FailFast::class)
...
//
//        `Should stop job creating savepoint and restart from savepoint when required`()
//
...
//
//        `Should stop cluster and jobs creating savepoint and restart from savepoint when required`()
//
...
//    }
    ...
    @Test
    @Order(1010)
    fun `Should start the cluster and its jobs when the cluster is created`() {
    ...
    @Test
    @Order(1020)
    fun `Should stop job creating savepoint and restart from savepoint when required`() {
    ...
    @Test
    @Order(1030)
    fun `Should trigger savepoint when requested`() {
    ...
    @Test
    @Order(1031)
    fun `Should forget savepoint when requested`() {
    ...
    @Test
    @Order(1040)
    fun `Should start the cluster and its jobs when cluster has multiple jobs`() {
    ...
    @Test
    @Order(1052)
    fun `Should return JobManager details`() {
    ...
    @Test
    @Order(1054)
    fun `Should return TaskManager details`() {
    ...
    @Test
    @Order(1055)
    fun `Should return TaskManager metrics`() {
    ...
    @Test
    @Order(1057)
    fun `Should delete cluster and jobs`() {
    ...
    @Test
    @Order(1060)
    fun `Should stop cluster and jobs creating savepoint and restart from savepoint when required`() {
    ...
    @Test
    @Order(1070)
    fun `Should recreate missing resources`() {
    ...
    @Test
    @Order(1100)
    fun `Should stop job when batch job has finished`() {
...
//        assertClusterDeleted(clusterName = "cluster-3")
    ...
    @Test
    @Order(1120)
    fun `Should restart a job but ignore the savepoint`() {
    ...
    @Test
    @Order(1130)
    fun `Should stop job when job fails because invalid bootstrap image`() {
    ...
    @Test
    @Order(1131)
    fun `Should stop job when job fails because invalid bootstrap className`() {
        ...
        if (updateDeployment(namespace = namespace, name = "cluster-2", patch = "[{\"op\":\"replace\",\"path\":\"/spec/jobs/0/spec/bootstrap/className\",\"value\":\"com.nextbreakpoint.flink.jobs.ComputeMaximum\"}]") != 0) {
            fail("Can't update deployment")
        }
    ...
    @Test
    @Order(1140)
    fun `Should start the cluster without jobs`() {
    ...
    @Test
    @Order(1150)
    fun `Should update clusters when servicemode changed`() {
    ...
    @Test
    @Order(1151)
    fun `Should update clusters when taskslots changed`() {
    ...
    @Test
    @Order(1152)
    fun `Should update clusters when taskmanagers changed`() {
    ...
    @Test
    @Order(1160)
    fun `Should update clusters when scaling taskmanagers down`() {
    ...
    @Test
    @Order(1161)
    fun `Should update clusters when scaling taskmanagers up`() {
    ...
    @Test
    @Order(1162)
    fun `Should update clusters when scaling taskmanagers down to zero`() {
    ...
    @Test
    @Order(1163)
    fun `Should update clusters when scaling job parallelism`() {
        ...
        awaitUntilAsserted(timeout = 480) {
            assertThat(hasResourceStatus(namespace = namespace, resource = "fc", name = "cluster-2", status = ResourceStatus.Updated)).isTrue()
            assertThat(hasLessOrEqualToTaskManagers(namespace = namespace, name = "cluster-2", taskManagers = 2)).isTrue()
            val latestStatus = getLatestClusterStatus(clusterName = "cluster-2")
            assertThat(latestStatus.taskManagers).isLessThanOrEqualTo(2)
            assertThat(latestStatus.taskManagerReplicas).isLessThanOrEqualTo(2)
            assertThat(latestStatus.taskSlots).isLessThanOrEqualTo(4)
            assertThat(latestStatus.totalTaskSlots).isLessThanOrEqualTo(4)
        }
    }

    @Test
    @Order(1170)
    fun `Should delete cluster with multiple jobs`() {
