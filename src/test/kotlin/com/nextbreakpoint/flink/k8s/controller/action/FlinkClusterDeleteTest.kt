...
class FlinkClusterDeleteTest {
    ...
    fun `should fail when kubeClient throws exception`() {
        given(kubeClient.deleteFlinkCluster(eq("flink"), eq("test"))).thenThrow(RuntimeException::class.java)
        val result = command.execute("flink", "test", null)
        verify(kubeClient, times(1)).deleteFlinkCluster(eq("flink"), eq("test"))
    ...
    @Test
    fun `should delete flink cluster`() {
        val result = command.execute("flink", "test", null)
        verify(kubeClient, times(1)).deleteFlinkCluster(eq("flink"), eq("test"))
        verifyNoMoreInteractions(kubeClient)
        verifyNoMoreInteractions(flinkClient)
