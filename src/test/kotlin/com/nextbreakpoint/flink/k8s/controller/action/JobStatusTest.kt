...
class JobStatusTest {
    ...
    private val command = JobGetStatus(flinkOptions, flinkClient, kubeClient)
    ...
    fun `should return expected result when it can't fetch job status`() {
        ...
        val result = command.execute("flink", "test", "test", "123")
    ...
    fun `should return expected result when it can fetch job status`() {
        val result1 = command.execute("flink", "test", "test", "123")
        val result2 = command.execute("flink", "test", "test", "123")
