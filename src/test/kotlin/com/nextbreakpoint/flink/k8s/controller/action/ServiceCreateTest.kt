...
class ServiceCreateTest {
    ...
    private val command = ServiceCreate(flinkOptions, flinkClient, kubeClient)

    @BeforeEach
    fun configure() {
        val service = V1ServiceBuilder().withNewMetadata().withName("xxx").endMetadata().build()
        given(kubeClient.createService(eq("flink"), any())).thenReturn(service)
    }

    @Test
    fun `should fail when kubeClient throws exception`() {
        given(kubeClient.createService(eq("flink"), any())).thenThrow(RuntimeException::class.java)
        val result = command.execute("flink", "test", service)
        verify(kubeClient, times(1)).createService(eq("flink"), any())
