...
class TaskControllerTest {
    ...
    private val taskController = TaskController(clusterSelector, cluster, resources, operationController)
    ...
    fun `should return number of jobmanager replicas`() {
        ...
        val context = TaskController(clusterSelector, cluster, resources, operationController)
    ...
    fun `should return true when bootstrap exists otherwise false`() {
        ...
        val newMediator = TaskController(clusterSelector, cluster, newResource, operationController)
    ...
    fun `should return true when savepoint is required otherwise false`() {
        ...
        given(operationController.currentTimeMillis()).thenReturn(currentTimeMillis + savepointInterval * 1000 - 1)
        Annotations.setWithoutSavepoint(cluster, false)
        Annotations.setDeleteResources(cluster, false)
        assertThat(taskController.isSavepointRequired()).isEqualTo(false)
        given(operationController.currentTimeMillis()).thenReturn(currentTimeMillis + savepointInterval * 1000 + 10000)
        Annotations.setWithoutSavepoint(cluster, false)
        Annotations.setDeleteResources(cluster, false)
    ...
    fun `should refresh status`() {
        ...
        verify(operationController, times(1)).updateStatus(eq(clusterSelector), eq(cluster))
        verify(operationController, times(1)).updateFinalizers(eq(clusterSelector), eq(cluster))
        verify(operationController, times(1)).updateAnnotations(eq(clusterSelector), eq(cluster))
    }

    @Test
    fun `should create bootstrap resource`() {
        taskController.initializeStatus()
        given(operationController.createBootstrapJob(eq(clusterSelector), any())).thenReturn(OperationResult(status = OperationStatus.OK, output = "xxx"))
        assertThat(taskController.createBootstrapJob(clusterSelector)).isNotNull()
        verify(operationController, times(1)).createBootstrapJob(eq(clusterSelector), any())
    }
...
//        assertThat(jobCapture.lastValue.spec.template.spec.containers[0].args.find { it == "SAVEPOINT_PATH" }).isNotNull()
    ...
    fun `should create jobmanager service resource`() {
        taskController.initializeStatus()
        given(operationController.createService(eq(clusterSelector), any())).thenReturn(OperationResult(status = OperationStatus.OK, output = "xxx"))
        assertThat(taskController.createService(clusterSelector)).isNotNull()
        verify(operationController, times(1)).createService(eq(clusterSelector), any())
    }

    @Test
    fun `should remove jar`() {
        given(operationController.removeJar(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = null))
        assertThat(taskController.removeJar(clusterSelector)).isNotNull()
        verify(operationController, times(1)).removeJar(eq(clusterSelector))
    }

    @Test
    fun `should trigger savepoint`() {
        given(operationController.triggerSavepoint(eq(clusterSelector), eq(savepointOptions))).thenReturn(OperationResult(status = OperationStatus.OK, output = savepointRequest))
        assertThat(taskController.triggerSavepoint(clusterSelector, savepointOptions)).isNotNull()
        verify(operationController, times(1)).triggerSavepoint(eq(clusterSelector), eq(savepointOptions))
    }

    @Test
    fun `should query savepoint`() {
        given(operationController.querySavepoint(eq(clusterSelector), eq(savepointRequest))).thenReturn(OperationResult(status = OperationStatus.OK, output = "file:///tmp/1"))
        assertThat(taskController.querySavepoint(clusterSelector, savepointRequest)).isNotNull()
        verify(operationController, times(1)).querySavepoint(eq(clusterSelector), eq(savepointRequest))
    }

    @Test
    fun `should create job`() {
        val resource = TestFactory.aBootstrapJob(cluster)
        given(operationController.createBootstrapJob(eq(clusterSelector), eq(resource))).thenReturn(OperationResult(status = OperationStatus.OK, output = "xxx"))
        assertThat(taskController.createBootstrapJob(clusterSelector, resource)).isNotNull()
        verify(operationController, times(1)).createBootstrapJob(eq(clusterSelector), eq(resource))
    }

    @Test
    fun `should delete job`() {
        given(operationController.deleteBootstrapJob(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = null))
        assertThat(taskController.deleteBootstrapJob(clusterSelector)).isNotNull()
        verify(operationController, times(1)).deleteBootstrapJob(eq(clusterSelector))
    }

    @Test
    fun `should create service`() {
        val resource = TestFactory.aJobManagerService(cluster)
        given(operationController.createService(eq(clusterSelector), eq(resource))).thenReturn(OperationResult(status = OperationStatus.OK, output = "xxx"))
        assertThat(taskController.createService(clusterSelector, resource)).isNotNull()
        verify(operationController, times(1)).createService(eq(clusterSelector), eq(resource))
    }

    @Test
    fun `should delete service`() {
        given(operationController.deleteService(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = null))
        assertThat(taskController.deleteService(clusterSelector)).isNotNull()
        verify(operationController, times(1)).deleteService(eq(clusterSelector))
    }

    @Test
    fun `should create pods`() {
        given(operationController.createPods(eq(clusterSelector), eq(podReplicas))).thenReturn(OperationResult(status = OperationStatus.OK, output = setOf("1", "2")))
        assertThat(taskController.createPods(clusterSelector, podReplicas)).isNotNull()
        verify(operationController, times(1)).createPods(eq(clusterSelector), eq(podReplicas))
    }

    @Test
    fun `should delete pod`() {
        given(operationController.deletePods(eq(clusterSelector), any())).thenReturn(OperationResult(status = OperationStatus.OK, output = null))
        assertThat(taskController.deletePods(clusterSelector, deleteOptions)).isNotNull()
        verify(operationController, times(1)).deletePods(eq(clusterSelector), eq(deleteOptions))
    }

    @Test
    fun `should verify that pods are running`() {
        given(operationController.arePodsRunning(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.arePodsRunning(clusterSelector)).isNotNull()
        verify(operationController, times(1)).arePodsRunning(eq(clusterSelector))
    }

    @Test
    fun `should verify that pods are terminated`() {
        given(operationController.arePodsTerminated(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.arePodsTerminated(clusterSelector)).isNotNull()
        verify(operationController, times(1)).arePodsTerminated(eq(clusterSelector))
    }


    @Test
    fun `should start job`() {
        given(operationController.startJob(eq(clusterSelector), eq(cluster))).thenReturn(OperationResult(status = OperationStatus.OK, output = null))
        assertThat(taskController.startJob(clusterSelector, cluster)).isNotNull()
        verify(operationController, times(1)).startJob(eq(clusterSelector), eq(cluster))
    }

    @Test
    fun `should stop job`() {
        given(operationController.stopJob(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.stopJob(clusterSelector)).isNotNull()
        verify(operationController, times(1)).stopJob(eq(clusterSelector))
    }

    @Test
    fun `should cancel job`() {
        given(operationController.cancelJob(eq(clusterSelector), eq(savepointOptions))).thenReturn(OperationResult(status = OperationStatus.OK, output = savepointRequest))
        assertThat(taskController.cancelJob(clusterSelector, savepointOptions)).isNotNull()
        verify(operationController, times(1)).cancelJob(eq(clusterSelector), eq(savepointOptions))
    }

    @Test
    fun `should verify that cluster is ready`() {
        given(operationController.isClusterReady(eq(clusterSelector), eq(clusterScale))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.isClusterReady(clusterSelector, clusterScale)).isNotNull()
        verify(operationController, times(1)).isClusterReady(eq(clusterSelector), eq(clusterScale))
    }

    @Test
    fun `should verify that job is running`() {
        given(operationController.isJobRunning(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.isJobRunning(clusterSelector)).isNotNull()
        verify(operationController, times(1)).isJobRunning(eq(clusterSelector))
    }

    @Test
    fun `should verify that job is finished`() {
        given(operationController.isJobFinished(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.isJobFinished(clusterSelector)).isNotNull()
        verify(operationController, times(1)).isJobFinished(eq(clusterSelector))
    }

    @Test
    fun `should verify that job is failed`() {
        given(operationController.isJobFailed(eq(clusterSelector))).thenReturn(OperationResult(status = OperationStatus.OK, output = true))
        assertThat(taskController.isJobFailed(clusterSelector)).isNotNull()
        verify(operationController, times(1)).isJobFailed(eq(clusterSelector))
    }

    @Test
    fun `should return time passed since last update`() {
        Status.setClusterStatus(cluster, ClusterStatus.Running)
        given(operationController.currentTimeMillis()).thenReturn(System.currentTimeMillis() + 5000)
        assertThat(taskController.timeSinceLastUpdateInSeconds()).isGreaterThanOrEqualTo(5)
        Status.setClusterStatus(cluster, ClusterStatus.Running)
        given(operationController.currentTimeMillis()).thenReturn(System.currentTimeMillis() + 10000)
