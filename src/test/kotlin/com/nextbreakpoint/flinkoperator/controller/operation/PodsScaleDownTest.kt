...
class PodsScaleDownTest {
    ...
    fun `should fail when kubeClient throws exception`() {
        given(kubeClient.terminateStatefulSets(eq(clusterSelector))).thenThrow(RuntimeException::class.java)
        val result = command.execute(clusterSelector, null)
        verify(kubeClient, times(1)).terminateStatefulSets(eq(clusterSelector))
