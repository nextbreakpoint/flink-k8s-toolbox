...
class ClusterResourcesBuilderTest {
    ...
    @BeforeEach
    fun setup() {
        given(factory.createBootstrapJob(any(), any(), any(), any())).thenReturn(bootstrapJob)
    ...
    fun `should invoke factory to create resources`() {
        ...
        verify(factory, times(1)).createBootstrapJob(eq("test"), eq(clusterId), eq("myself"), eq(cluster))
