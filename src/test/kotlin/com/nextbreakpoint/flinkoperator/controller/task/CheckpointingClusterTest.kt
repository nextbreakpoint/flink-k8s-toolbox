...
class CheckpointingClusterTest {
    ...
    private val task = CheckpointingCluster()
    ...
    fun `onExecuting should update cluster status`() {
        ...
        assertThat(result).isNotNull()
        assertThat(result.status).isEqualTo(ResultStatus.SUCCESS)
        assertThat(OperatorAnnotations.getClusterStatus(cluster)).isEqualTo(ClusterStatus.CHECKPOINTING)
