...
class DeleteResourcesTest {
    ...
    fun `onExecuting should return expected result when resources can't be deleted`() {
        given(context.deleteClusterResources(eq(clusterId))).thenReturn(OperationResult(OperationStatus.FAILED, null))
        val result = task.onExecuting(context)
    ...
    fun `onExecuting should return expected result when resources have been deleted`() {
        given(context.deleteClusterResources(eq(clusterId))).thenReturn(OperationResult(OperationStatus.COMPLETED, null))
        val result = task.onExecuting(context)
        verify(context, atLeastOnce()).clusterId
        verify(context, atLeastOnce()).flinkCluster
        verify(context, atLeastOnce()).timeSinceLastUpdateInSeconds()
